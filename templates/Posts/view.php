    ...
    <div class="column column-80">
        <div class="posts view content">
            ...
            <table>
                ...
                <tr>
                    <th><?= __('Project') ?></th>
                    <td><?= $post->hasValue('project') ? $this->Html->link($post->project->title, ['controller' => 'Projects', 'action' => 'view', $post->project->id]) : '' ?></td>
                </tr>
                <tr>
                    <th><?= __('User') ?></th>
            ...
            <div class="related">
                ...
                <div class="table-responsive">
                    <table>
                        ...
                        <?php foreach ($post->post_photos as $postPhoto) : ?>
                        <tr>
                            <td><?= h($postPhoto->id) ?></td>
                            <td><?= h($postPhoto->title) ?></td>
                            <td><?= h($postPhoto->caption) ?></td>
                            <td><?= h($postPhoto->path) ?></td>
                            <td><?= h($postPhoto->created) ?></td>
                            <td><?= h($postPhoto->post_id) ?></td>
                            <td><?= h($postPhoto->sort_order) ?></td>
                            <td class="actions">
                                <?= $this->Html->link(__('View'), ['controller' => 'PostPhotos', 'action' => 'view', $postPhoto->id]) ?>
                                <?= $this->Html->link(__('Edit'), ['controller' => 'PostPhotos', 'action' => 'edit', $postPhoto->id]) ?>
                                <?= $this->Form->postLink(
                                    __('Delete'),
                                    ['controller' => 'PostPhotos', 'action' => 'delete', $postPhoto->id],
                                    [
                                        'method' => 'delete',
